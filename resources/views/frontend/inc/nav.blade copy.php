<style>
    .primary-nav-section{
        /* background-image: linear-gradient(to right, #CB356B, #E88139); */
        background-image: linear-gradient(to right, #3264af, #ec252d);
    }
</style>
        @php
...
{{--                        </li>--}}
                        ...
                        <li class="dropdown" id="currency-change">
                            @php
                                $code = \App\Models\Currency::findOrFail(\App\Models\BusinessSetting::where('type', 'home_default_currency')->first()->value)->code;
                    ...
                    <ul class="inline-links">
                        <li>
                            @if($generalsetting->phone != null)
                                <a href="tel:{{$generalsetting->phone}}" class="top-bar-item">conrtact: {{ $generalsetting->phone }}</a>
                            @endif
                        </li>
                        <li>
                            @if($generalsetting->email != null)
                            <a href="mailto:{{$generalsetting->email}}" class="top-bar-item">Email: {{ $generalsetting->email }}</a>
                            @endif
                        </li>
                        @auth
                        <li>
                        ...
                        @else
                        <li>
                            <a href="{{ route('user.login') }}" class="top-bar-item">{{__('Sign In')}}</a>
                        </li>
                        <li>
                        ...
                        <li>
                            <button class="">
                                <a href="{{ route('user.ordernow') }}" class="top-bar-item">{{__('Order Now')}}</a>
                            </button>
                        </li>
                    </ul>
                </div>
                ...
                <div class="side-menu-list px-3">
                    <ul class="side-user-menu">
                        ...
                        <li>
                            ...
                            <a href="{{route('usermedication.view')}}">
                            ...
                            @else
                            <a href="{{ route('usermedication.index') }}">
                                <i class="fa fa-medkit"></i>
                                <span class="category-name">
                                    {{__('Add Medication')}}
                                </span>
                        ...
                        <div class="widget-balance py-3">
                            <div class="text-center">
                                <div class="heading-4 strong-700 mb-4">
                                    @php
                                        $orderDetails = \App\Models\OrderDetail::where('seller_id', Auth::user()->id)->where('created_at', '>=', date('-30d'))->get();
                                        $total = 0;
                                ...
                                <table class="text-left mb-0 table w-75 m-auto">
                                    <tbody>
                                        ...
                                        <tr>
                                            @php
                                                $orderDetails = \App\Models\OrderDetail::where('seller_id', Auth::user()->id)->where('created_at', '>=', date('-60d'))->where('created_at', '<=', date('-30d'))->get();
                                                $total = 0;
                                                foreach ($orderDetails as $key => $orderDetail) {
                                                    if($orderDetail->order->payment_status == 'paid'){
                                                        $total += $orderDetail->price;
                    ...
                    <div class="col-lg-3 col-10">
                        <div class="d-flex">
                            ...
                            <a class="navbar-brand w-100" href="{{ route('home') }}">
                                ...
                                @else
                                    <img src="{{asset('./files/Logo_orange.png')}}" class="" alt="active shop">
                                @endif
                            </a>
{{--
                                                        ...
                                                        <div class="sub-cat-brand">
                                                            <ul class="sub-brand-list">
                                                                @foreach ($brands as $brand_id)
                                                                    @if(\App\Models\Brand::find($brand_id) != null)
                                                                        <li class="sub-brand-item">
                                                                            <a href="{{ route('products.brand', $brand_id) }}" ><img src="{{ asset(\App\Models\Brand::find($brand_id)->logo) }}" class="img-fluid"></a>
                                                                        </li>
                                                                    @endif
                                                                @endforeach
                                                            </ul>
            ...
            <div class="container">
                <div class="row">
                    ...
                    <ul class="nav primary clone-main-menu" id="mercado_main" data-menuname="Main menu">
                        ...
                        {{--                    <li class="menu-item home-icon">--}}
                        {{--                        <a href="{{url('/')}}" class="link-term mercado-item-title"><span style="color: white;font-weight: bold">Home</span></a>--}}
                        {{--                    </li>--}}

                        @foreach (\App\Models\Category::all()->take(8) as $key => $category)
                            @php
                                $brands = array();
                            @endphp
                            <li class="menu-item menu-item-has-children item-megamenu">
                                <a href="{{ route('products.category', $category->id) }}" title="{{ __($category->name) }}" class="link-term mercado-item-title">{{ __($category->name) }}</a>
                                @if(count($category->subcategories)>0)
                                                ...
                                                @foreach ($category->subcategories as $subcategory)
                                                    <div class="custom-col-5 {{$i%2==0?'bgWhite':'bgGrey'}}">
                                                        <div class="wrap-vertical-nav">
                                                            <a href="{{ route('products.subcategory', $subcategory->id) }}"><h2 class="menu-title">{{ __($subcategory->name) }}</h2></a>
                                                            <ul>
                                                                @foreach ($subcategory->subsubcategories as $subsubcategory)
                                                                    @php
                                                                        foreach (json_decode($subsubcategory->brands) as $brand) {
                                                                    ...
                                                                    @endphp
                                                                    <li class="menu-item"><a href="{{ route('products.subsubcategory', $subsubcategory->id) }}">{{ __($subsubcategory->name) }}</a></li>
                                                                @endforeach
                                                            </ul>
                                                        </div>
                                                    </div>
                    ...
                    <a href="{{route('products')}}" style="margin: 0 10px 0 100px;padding-top: 11px;color: white">Shop</a>
                    <!--<a href="{{route('front.blog')}}" style="margin: 0 10px; padding-top: 11px;color: white">Blog</a>-->

                    <!--<ul class="nav primary clone-main-menu" id="mercado_main" data-menuname="Main menu">-->
                    <!--    <li class="menu-item menu-item-has-children item-megamenu">-->
                    <!--        <a href="{{route('front.doctor')}}"  style="padding-top: 11px;color: white">Doctor</a>-->
                    <!--            <div class="wrap-megamenu" style="width: 300px;">-->
                    <!--                <div class="megamenu-content">-->
                    <!--                            <div class="custom-col-5">-->
                    <!--                                <div class="wrap-vertical-nav">-->
                    <!--                                    <ul>-->
                    <!--                                        <li>-->
                    <!--                                        @foreach (\App\Models\DocDepartment::all() as $key => $dep)-->
                    <!--                                        <a href="{{ route('front.department', encrypt($dep->id)) }}">{{ __($dep->dep_name) }}</a>-->
                    <!--                                        @endforeach-->
                    <!--                                        </li>-->
                    <!--                                    </ul>-->
                    <!--                                </div>-->
